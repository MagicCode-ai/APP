...
internal class SimulcastVideoEncoderFactoryWrapper(
    ...
    private class StreamEncoderWrapperFactory(private val factory: VideoEncoderFactory) :
        ...
        override fun createEncoder(videoCodecInfo: VideoCodecInfo?): VideoEncoder? {
            ...
            return StreamEncoderWrapper(wrapH264Extras(encoder, videoCodecInfo))
        }
...
private fun wrapH264Extras(encoder: VideoEncoder, info: VideoCodecInfo?): VideoEncoder {
    val name = info?.name ?: return encoder
    if (!name.equals("H264", ignoreCase = true)) {
        return encoder
    }
    var wrapped = encoder
    wrapped = wrapIfEnabled(wrapped, "com.videocall.video_call.MagicStreamingVideoEncoder")
    return wrapped
}

private fun wrapIfEnabled(encoder: VideoEncoder, className: String): VideoEncoder {
    return try {
        val cls = Class.forName(className)
        val enabled = cls.getMethod("isEnabled").invoke(null) as? Boolean ?: false
        if (!enabled) {
            return encoder
        }
        cls.getConstructor(VideoEncoder::class.java).newInstance(encoder) as VideoEncoder
    } catch (_: Throwable) {
        encoder
    }
}
